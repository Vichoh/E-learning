...
<div class="container ml-0 pl-0 pr-0 mt-5" style="width: 100%;">
    ...
    <h4 class="card-title text-white pt-4 text-center">Licencias</h4>
      ...
      <?php if ($this->user_level == 2 ){ ?>
      <a href="<?php echo Config::get('URL'); ?>licencia/showUsersTeacher" class="list-group-item list-group-item-action bg-nav text-white active" ><i class="fa fa-user text-success pr-2"></i> Usuarios por Licencia</a>
      <?php } ?>
        ...
        function grupos(){
            ...
            $.getJSON(appUrl + 'licencia/getUsersLicence/' + licencia, function(data) {
                ...
                data.forEach(function (element) {
                    ...
                    div += ' <tr><th class="style-tarjetas">' + count +'</th><th class="style-tarjetas">' + element.first_name + " " + element.last_name +  '</th><th class="style-tarjetas">'+ element.user_email+'  </th> <th class="style-tarjetas"><form method="post" action="<?php echo Config::get('URL'); ?>licencia/solicitudEliminacion"><input type="hidden" name="id" value="'+element.lu_id+'" /><input type="hidden" name="estado" value="1" /><button class="btn btn-success" ><input type="submit" style="background-color: transparent; border-color: transparent;" class="text-white"value="Solicitud de eliminacion" /></button></form></th> </tr>'
             ...
             $("button").click(function(){
              var id = $ ('#buttonSolicitud').val();
              console.log(id);
              var appUrl = '<?php echo Config::get('URL'); ?>';
              $.post(appUrl + 'licencia/solicitudEliminacion/' + id, {estado: 1}, function(result){
                  console.log(result);
              });
            });
