<section class="mb-0" >
    <div class="container pt-5">
        <div class="row">
            <div class="col-12 login mt-3 bd-primary pr-0 pl-0 borde-encuadre" style="background-color: white ">
                <h5 class="bg-primary pt-2 pb-2 pl-2 pr-2 text-center text-white">Perfil</h5>
                ...
                <div class="row pr-2 pl-2">
                    <div class="col-4 pb-2">
                        <div class="card text-center">
                            <div class="text-center pt-2 image_wrapper">
                                <?php if (Config::get('USE_GRAVATAR')) { ?>
                                    <img alt="Avatar" class="img-circle text-center" src='<?= $this->user_gravatar_image_url; ?>' style="height: 100px; width: 100px"/>='<?= $this->user_avatar_file; ?>' style="height: 100px; width: 100px"/>
                                    <?php } else { ?>
                                    <a href="<?php echo Config::get('URL'); ?>user/editAvatar"><img alt="Avatar" class="text-center image" src='<?= $this->user->user_avatar_link; ?>' style="height: 200px; width: 200px"/></a>
                                    <?php } ?>
                                </div>
                                <div class="card-block">
                                    <h4 class="card-title text-primary text-justify "><?= $this->user->user_name; ?>
                                    ...
                                    <div class="text-center pb-2">
                                            <div class="col">
                                            <label for="nameUser">Frase <?php if ($this->admin == $this->member) { ?><a onclick="editFrase()"><i class="fa fa-pencil" name="btnFrase" aria-hidden="true"></i></a><?php } ?></label>
                                            <form action="<?php echo Config::get('URL'); ?>user/editUserSentence_action" method="post">
                                                <div class="input-group">
                                                    <div class="input-group-addon"><i class="fa fa-address-book" aria-hidden="true"></i></div>
                                                    <input autofocus required class="form-control" maxlength="100" type="text" id="frase_user" name="new_frase" placeholder="Ingrese su frase" readonly="false" value="<?= $this->user->user_sentence; ?>"/>
                                                    <button class="btn btn-success text-right" name="btnEFrase" onclick="editFrase()" style="display: none"><i class="fa fa-floppy-o" aria-hidden="true"></i></button>
                                                </div>
                                            </form>
                                        </div>
                                    </div>
                      ...
                      <div class="col mb-3 text-right">
                        ...
                        <a href="<?php echo Config::get('URL'); ?>dashboard" class="btn btn-success"> <i class="fa fa-chevron-left" aria-hidden="true"></i> Volver </a>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function editNombre(){
     ...
     function editFrase(){
        var frase = document.getElementById("frase_user");
        var btnFrase = document.getElementsByName("btnFrase");
        var btnEFrase = document.getElementsByName("btnEFrase");
        if (frase.readOnly === false) {
            frase.readOnly = true;
            btnFrase[0].style.display = 'block';
            btnEFrase[0].style.display = 'none';
        } else {
            frase.readOnly = false;
            btnFrase[0].style.display = 'none';
            btnEFrase[0].style.display = 'block';
        }
    }
</script>
